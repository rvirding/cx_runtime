...
-module(mandelbrot_no_export).
-export([start/0, main/1, start_row/7]).
-define(LIM_SQR, 4.0).
-define(ITER, 50).
-define(SR, -1.5).
-define(SI, -1).
...
main(N) ->
    case file:open("/tmp/output.ppm", [write]) of
        {ok, F} ->
            ...
            Row = fun(Y)-> spawn(mandelbrot_no_export, start_row, [F, 0, ?SI+Y*2/N, N, 0, [], 7]) end,
...
start_row(F, X, Y2, N, Bits, Bytes, BitC) ->
	row(F, X, Y2, N, Bits, Bytes, BitC).
